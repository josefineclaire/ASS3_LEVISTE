...
dob.addEventListener('input', () => validateInput(dob));

// Handle form submission
form.addEventListener('submit', (e) => {
    e.preventDefault(); // Prevent default form submission
    
    if (checkInputs()) { // If all inputs are valid, show the success message
...
function checkInputs() {
    ...
    if (!validateInput(dob)) isValid = false;
...
function validateInput(input) {
    ...
    if (input === firstName && value === '') {
        setErrorFor(input, 'First Name cannot be blank');
        isValid = false;
